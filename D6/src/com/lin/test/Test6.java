package com.lin.test;

/**
 * @author ：lin
 * @date ：Created in 2025/1/20
 * @description ：
 * @version: 1.0
 */
public class Test6 {
    public static void main(String[] args) {
/*
        A numeric password (greater than 0) for a particular system. 1983, for example, is transmitted using the encryption
        The rules are as follows:
            Add 5 to each digit
            And then by adding the remainder to 10.
            And finally invert all the digits to
            To get a new string of numbers.
*/


        // Analyze:
        // 1. Put each bit of the integer into an array.
        int[] arr = {1, 9, 8, 3};
        // 2. Encryption
        // Add 5 to each digit
        for (int i = 0; i < arr.length; i++) {
            arr[i] = arr[i] + 5;
        }
        // And then the remainder of 10.
        for (int i = 0; i < arr.length; i++) {
            arr[i] = arr[i] % 10;
        }
        // Invert all numbers
        for (int i = 0, j = arr.length - 1; i < j; i++, j--) {
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
        // 8 3 4 6 --> 8346
        // 3. Splice each number in the array into the encrypted result
        int number = 0;
        for (int i = 0; i < arr.length; i++) {
            number = number * 10 + arr[i];
        }
        System.out.println(number);
    }
}
